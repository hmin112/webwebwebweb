...
// team.members(팀 탭에서 이미 불러온 값)로 곧바로 채워진다 — 별도 조회가 필요 없다.
export const TeamPlanPage = ({
  loginId,
  teamId,
  team,
  submission,
  ...
  const disabled = !submission.isWithinPeriod;
  const teamMembers = useMemo(
    () => (team?.members || []).filter((m: any) => m.status === "ACCEPTED"),
    [team]
  );
  ...
  const submissionIdRef = useRef<string>(submission.id?.toString() || "0");

  const buildPayload = (s: PlanState) => ({
    loginId,
    teamId,
    submissionId: submissionIdRef.current.includes("temp") ? "0" : submissionIdRef.current,
  ...
  const saveNow = async (s: PlanState) => {
    ...
    setSaveState("saving");
    try {
      const res = await api.post("/team-submissions/plan/save", buildPayload(s));
      if (res.data?.id) submissionIdRef.current = res.data.id.toString();
      ...
      <div className="flex gap-3 mt-6">
        ...
        {!disabled && (
          ...
          >
            ...
            {isSubmittedStatus(submission.status) ? "제출 내용 다시 확정" : "제출 확정"}
          </button>
        )}
      </div>
    </motion.div>
