...
export const FindAccount = ({ onNavigate }: { onNavigate: (page: string) => void }) => {
  ...
  const handleRequestDM = async () => {
    ...
    try {
      ...
      const botRes = await api.post("/members/discord-send", { discordTag });
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (activeTab === "id") {
        ...
        setResetVerificationToken(res.data.verificationToken);
      }
  ...
  const handleResetPassword = async () => {
    ...
    if (!resetVerificationToken) {
      return alert("인증 토큰이 없습니다. 다시 인증해주세요.");
    }
        ...
        <div className="bg-white rounded-[40px] shadow-2xl shadow-indigo-100/50 border border-slate-100 overflow-hidden">
          <div className="flex bg-slate-50/50 p-2 gap-2">
            <button
              ...
              className={`flex-1 py-4 rounded-3xl font-bold text-sm transition-all ${activeTab === "id" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400"}`}
            >
              아이디 찾기
            </button>
            <button
              ...
              className={`flex-1 py-4 rounded-3xl font-bold text-sm transition-all ${activeTab === "pw" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400"}`}
            >
              비밀번호 재설정
            </button>
          </div>
          ...
          <div className="p-10">
            {!isVerified ? (
              <form className="space-y-6" onSubmit={handleVerify}>
                ...
                <p className="text-slate-500 font-medium mb-8">디스코드 DM 인증으로 본인 확인을 진행합니다.</p>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="홍길동"
                        className="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500 font-medium"
                      />
                    </div>
                  </div>
                    ...
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          className="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl outline-none text-center tracking-widest font-bold"
                        />
                      </div>
                ...
                <Button type="submit" className="w-full py-7 rounded-3xl bg-indigo-600 text-white font-black text-lg shadow-xl shadow-indigo-100 mt-4">
                  인증 및 확인
                </Button>
              </form>
            ) : (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-6">
                ...
                {activeTab === "id" ? (
                  <>
                    ...
                    <Button onClick={() => onNavigate("login")} className="w-full py-6 rounded-3xl bg-slate-900 text-white font-bold">
                      로그인하러 가기
                    </Button>
                  </>
                ) : (
                  <>
                    ...
                    <div className="space-y-3 mb-8 text-left">
                      <input
                        ...
                        placeholder="새 비밀번호"
                        className="w-full px-6 py-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500 font-medium"
                      />
                      <input
                        type="password"
                    ...
                    <Button onClick={handleResetPassword} className="w-full py-6 rounded-3xl bg-indigo-600 text-white font-bold">
                      비밀번호 변경 완료
                    </Button>
                  </>
                )}
